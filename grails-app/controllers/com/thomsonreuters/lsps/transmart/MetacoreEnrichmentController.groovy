...
class MetacoreEnrichmentController {
	...
	def prepareData = {
		...
		jobData['jobTmpDirectory'] = grailsApplication.config.RModules.tempFolderDirectory + '/' + params['jobName']
		
		FileWriterUtil f = new FileWriterUtil();
		File baseDir = new File(grailsApplication.config.RModules.tempFolderDirectory)
